...
const Vehicle = require("../models/vehicle"); // ADDED: To verify vehicle exists
...
router.post("/", authenticate, authorize("admin"), async (req, res) => {
  try {
    ...
    // THE FIX: Strict Backend Validation. Ensure vehicle is in THIS admin's fleet.
    const vehicleExists = await Vehicle.findOne({ regNumber: vehicleReg, adminId: req.user.id });
    if (!vehicleExists) {
        return res.status(400).json({ error: "Action Blocked: Vehicle not found in your registered fleet." });
    }
...
router.get("/daily-revenue", authenticate, async (req, res) => {
  try {
    ...
    const objectIdAdmin = new mongoose.Types.ObjectId(req.user.id);

    const result = await TransitLog.aggregate([
      { $match: { 
          adminId: objectIdAdmin, // Fixed casting here as well
